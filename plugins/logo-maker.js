...
cmd({
  pattern: "ثريدي",
  desc: "اصنع لوجو 3D ستايل كوميكس ✨",
  category: "logo",
  react: "🌀",
  filename: __filename
  ...
  try {
    if (!args.length) {
      return reply("❌ اكتبلي اسم اللوجو يا زعيم! مثال:\n*ثريدي لوسيفر*");
    }

    const name = args.join(" ");

    // API من موقع ephoto360
    ...
    if (!result?.result?.download_url) {
      return reply("❌ معرفتش أطلع الصورة، جرب تاني بعد شوية.");
    }

    await conn.sendMessage(from, {
      image: { url: result.result.download_url },
      caption: `🌀 *اللوجو بتاعك جاهز يا معلم*\n\n👑 *الاسم:* ${name}\n\n✨ *بتوقيع ✪『𝐋𝐔𝐂𝐈𝐅𝐄𝐑』✪*`
    });

  } catch (e) {
    return reply(`❌ حصلت مشكلة يا كبير:\n${e.message || "السبب مش معروف 😅"}`);
  }
});
...
cmd({
  pattern: "دراجون",
  desc: "اصنع لوجو ستايل دراجون بول 🔥",
  category: "logo",
  react: "🐉",
  filename: __filename
}, async (conn, mek, m, { from, args, reply }) => {
  try {
    if (!args.length) {
      return reply("❌ اكتبلي اسم اللوجو يا عِم 👑\nمثال: *دراجون لوسيفر*");
    }
    ...
    if (!result?.result?.download_url) {
      return reply("❌ معرفتش أعمل اللوجو، جرب تاني بعدين.");
    }

    await conn.sendMessage(from, {
      image: { url: result.result.download_url },
      caption: `🐉 *اللوجو الجامد بتاعك اهو يا كبير*\n\n💥 *الاسم:* ${name}\n\n✨ *بتوقيع ✪『𝐋𝐔𝐂𝐈𝐅𝐄𝐑』✪*`
    });

  } catch (e) {
    return reply(`❌ حصلت مشكلة وانا شغال:\n${e.message || "مش قادر أحدد المشكلة 😅"}`);
  }
});
